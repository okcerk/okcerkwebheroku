...
class MainPage extends Component {
    constructor(props) {
        ...
        const initialItems = this.props.pageContentListLeft.slice(0, ContentPageSize);
        this.state = {
            items: initialItems,
            hasMoreItems: true,
        };
    ...
    loadItems = (page) => {
        ...
        if (this.state.hasMoreItems && (ContentPageSize * requestedPage <= this.props.pageContentListLeft.length + ContentPageSize)) {
            this.setState({ items: this.props.pageContentListLeft.slice(0, ContentPageSize * requestedPage) });
        } else {
    ...
    render() {
        ...
        const pageContentItems = this.state.items.map(function (pageContent, index) {
            return (
                <PageContent
                    key={pageContent.key}
